...
class InvoiceListView(ListView):
    model = Invoice
    template_name = 'shipping/invoice_list.html'  # This is the correct path in the shipping app's templates
...
def edit_invoice(request, invoice_id):
    ...
    invoice = get_object_or_404(
        Invoice.objects.select_related(
            ...
            'shipment__sender_address__user'
        ),
        id=invoice_id
    )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Only update customer from shipment if not already set
            if not invoice.customer and invoice.shipment and invoice.shipment.sender_address:
                if hasattr(invoice.shipment.sender_address, 'user'):
                    invoice.customer = invoice.shipment.sender_address.user
            invoice.save()
        ...
        # Set initial customer value based on priority:
        # 1. Existing customer on invoice
        # 2. Shipment's sender user
        if invoice.customer:
            form.initial['customer'] = invoice.customer.id
        elif invoice.shipment and invoice.shipment.sender_address:
            if hasattr(invoice.shipment.sender_address, 'user') and invoice.shipment.sender_address.user:
                form.initial['customer'] = invoice.shipment.sender_address.user.id
    
    # Add debug information to template context
    debug_info = {
        'has_customer': bool(invoice.customer),
        'has_shipment': bool(invoice.shipment),
        'has_sender_address': bool(getattr(invoice, 'shipment', None) and invoice.shipment.sender_address),
        'has_sender_user': bool(getattr(getattr(getattr(invoice, 'shipment', None), 'sender_address', None), 'user', None))
    }
